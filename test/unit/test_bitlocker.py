...
''', SIG1=signature1, SIG2=signature2, SUSPENDED=suspended, LISTING=listing)
    ...
    def test_planning_protects_bitlocker_disk_before_asking_to_continue(self):
        result = self.run_shell(functions("prepare_bitlocker_free_space_install", "run_partition_decide") + r'''
disk=/dev/fake
protected_disk=""
...
''')
    ...
    def test_inspection_failure_names_partition_and_returns_without_writes(self):
        result = self.run_shell(functions(
            "check_bitlocker", "prepare_bitlocker_free_space_install"
        ) + r'''
disk=/dev/fake
step() { :; }
say() { printf '%s\n' "$*"; }
lsblk() { printf '/dev/fake disk\n/dev/fake1 part\n'; }
od() { return 1; }
gum() { return 0; }
protect_existing_partitions() { echo WRITE; }
prepare_bitlocker_free_space_install
''')
        self.assertEqual(result.returncode, 1)
        self.assertIn("/dev/fake1", result.stdout)
        self.assertNotIn("WRITE", result.stdout)

    def test_insufficient_source_disk_reaches_guarded_editor_before_bitlocker_gate(self):
        result = self.run_shell(functions("run_partition_decide") + r'''
disk=/dev/fake
is_install_media_disk() { return 0; }
verify_install_media() { return 0; }
prepare_bitlocker_free_space_install() { echo BITLOCKER; return 1; }
step() { :; }
partprobe() { :; }
sleep() { :; }
lsblk() { echo gpt; }
parted() { :; }
not_enough_space() { echo GUARDED_EDITOR; return 1; }
run_partition_decide
''')
        self.assertEqual(result.returncode, 1)
        self.assertIn("GUARDED_EDITOR", result.stdout)
        self.assertNotIn("BITLOCKER", result.stdout)
